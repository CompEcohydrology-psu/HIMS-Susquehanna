...
Resnut<-function(resflwo,res_vol,ressa,tmpav,varoute_4,varoute_5,varoute_6,
  ...
  psetlr1 <- routingParams$psetlr1
  
  psetlr2<-routingParams$psetlr2
  
  nsetlr1 <- routingParams$nsetlr1
  
  nsetlr2 <- routingParams$nsetlr2
  ...
  seccir <- routingParams$seccir
  
  res_solp  <- routingParams$res_solp 
  
  res_orgp <- routingParams$res_orgp
  
  res_no3 <- routingParams$res_no3
  
  res_no2 <- routingParams$res_no2
  
  res_nh3 <- routingParams$res_nh3
  
  res_orgn <- routingParams$res_orgn
  ...
  nitrok <- ressa * 10000. * (conc_n - con_nirr) * 
    ...
    phosk <- psetlr[iseas] * ressa * 10000. / 
      
                                          (res_vol + resflwo)
  
  nitrok <- nsetlr[iseas] * ressa * 10000. /  

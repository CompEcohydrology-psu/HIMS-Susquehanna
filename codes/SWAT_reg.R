...
# Module1: Coarse-Scale Landscape Simulation.

#This code perform simulates landscape process for multiple subbasins in ...
...
num <- 911       # number of SWAT subbasins (Susquehanna has 911 subbasins at HUC12 scale watershed)


dir1 <- ".../model"                                         # Model input files directory

dir2 <- ".../runfiles"                                      # Common files (runfiles folder) ... 

#for running the mdoel (these input files shared between subbasins)

dir3 <- ".../swat"                                          # Output storage directory

dir4 <- ".../codes"                                         # directory of developed codes

res_option <- 0                                             # not used for NHD+ routing

max_jobs <- 90  # maximum number of submitting jobs         # needs to be adjusted based on HPC resources
  ...
  for(i in remaining){  
    ...
    if(length(flist)>0){                                                               # if routing output is available
      ...
      path<-paste(dir3,'/S',i,sep="") 
        ...
        system('sbatch SWAT.pbs') # Running a Batch File in SLURM
